...
- **检索：FTS5 + BM25**。TG 索引把中文展开成 **CJK 二元组（bigram）** 写进 FTS5（Lucene `CJKBigramFilter` / ES `cjk` 分析器的做法），检索走倒排索引并用内置 `bm25()` 排序。实测比 `LIKE '%词%'` 全表扫描快 **6×~1500×**（典型查询 <10ms，原来 300~1000ms），62.8 万条消息的索引也终于能随规模扩展。旧库未建索引时自动回退 `LIKE`，`pansearch index build-fts` 一次性迁移（之后由触发器增量维护）。
- **匹配：轻量查询理解 + 角色分层**。查询词分「主题词 / 限定词（画质、格式）/ 通用词（破解、下载、合集、网盘）」三类，**通用词不能充当主题词** —— 这是 BM25 里 IDF≈0 的等价处理。实测「MATLAB 破解」原来把电视剧《河神之渡阴》排第一（只因正文里有"破解"），现在只留主题词命中的结果。无空格连写也会被切开（`AutoCAD破解版 → autocad + 破解版`），且不会误切 `HDMI`/`115`/`4K`。
- **排序：RRF 多源融合**。同一资源在 TG 的 BM25 序、站点序、网页序里都排前面时更可信（`Σ 1/(60+rank)`），比单纯"命中次数"更细。
- **归一：繁→简 + 全角/半角 + 零宽字符**。搜「周杰倫 無損」能命中标题写着「周杰伦 无损」的分享（可选依赖 `opencc-purepy`，装不上自动降级）。标点、引号、书名号、句末标点一律当分隔符 —— 从网页粘标题（`"三体"`、`《三体》全集`、`三体!`）不会再 0 结果。
- **保留相关结果**：同一链接保留各来源标题，选择最相关的标题展示；完整匹配优先于仅部分匹配，补搜得到的完整匹配不扣分。已验活的无关链接不能靠状态挤到相关结果前面。
- **有证据才过滤**：「只看存活」默认过滤确认失效，以及有标题但所有主题词都缺失的结果。缺标题、通用标题、未知跨语言标题、验活超时和不支持校验的结果继续保留；部分匹配降低排序。`--all` 可检查完整候选，`--strict` 仍按用户选择剔除无法验证的结果。
- **减少无效工作**：不再单独补搜 `4K HDR` / `全集`；TG 本地检索仅扫一次主查询（等价别名另查），查询限额仅用于含资源链接的消息。索引非空时，关键词未命中不会触发全频道重抓。垂直站点使用目录的分类路由，不再让固定软件站列表绕过路由。
- **保留超时前的收获**：站点、搜索引擎及 API 源超时时，已完成页面产生的命中仍会进入去重、排序和验活。
- **联网验活有预算**：先读取全部候选的有效缓存，再校验最多 150 条未缓存资源；缓存、API 直链和不支持校验的网盘不占这个名额。默认验活阶段最多等待 8 秒，未完成的标为未校验；`verify.budget: 0` 可全量校验，`verify.deadline: 0` 可关闭时间上限。8 秒是验活等待上限，不是整次搜索上限。

### 可靠性不变量（2026-09-13）

搜索引擎级的可靠性不是"有更多数据"，而是**几条能被验证的硬约束**。这些现在都有代码与测试兜着：

| 不变量 | 实现 | 验证 |
|---|---|---|
| **不静默降级** | `SearchOutcome.source_report` 记录每个源的状态/耗时/产出，`degraded` 列出失败/超时/部分失败的源；CLI 摘要与 Web 顶部都会显式告警"本次结果可能不全" | `tests/test_reliability.py::test_source_report_marks_failed_source` |
| **单点不致命** | PanSou 多实例：失败实例进 120s 健康度黑名单并排到最后（不永久拉黑，可自愈）；自建实例优先 | `test_pansou_failover_and_health_cache` |
| **重复搜索一致** | 查询级缓存（关键词+参数为键，TTL 300s，命中返回深拷贝）；**源故障导致的空结果不缓存**，允许重试 | `test_repeat_search_is_served_from_cache` / `test_cache_does_not_freeze_failed_empty_results` |
| **死源不白耗** | `websearch` 已关闭（实测 7.7s / 0 条），想恢复先换 Brave / SearXNG | 抽样：websearch 从每次超时变为不再参与 |
| **有回归门槛** | `scripts/fulltest.py --gate`：有结果率 ≥95%、top20 无关 ≤1%、异常 = 0 才退出 0（≤1% 是给"跨语言/无标题证据"这类刻意保留留的余量） | 见下方"全量回归" |

页面会分别显示无关过滤数与超时待验数，API 另提供预算跳过数、阶段耗时、`degraded` 与 `source_report`。以下实测记录反映历史版本，不代表当前网络下的耗时保证。

**全量回归（2026-09-13，`scripts/fulltest.py --gate`，200 个跨 12 领域搜索词）**

| 场景 | 有结果率 | top20 无关 | 异常 | 耗时 | 门槛 |
|---|---|---|---|---|---|
| 本地 TG 索引（离线） | **98.5%** | 0.42% | 0 | 221s | ✅ PASS |
| 全源（含网络） | **99.5%** | 0.89% | 0 | 787s | ✅ PASS |

全源失败统计：`pansou` 114/200、`sitesearch` 44/200 —— 这些现在会以"⚠ 召回降级"显式告警，而不是静默少结果。检索层实测比改造前快 6×~1500×（TG 检索 <10ms，原 `LIKE` 300~1000ms）。
