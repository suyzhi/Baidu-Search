...
### 4️⃣ 排序里"状态"和"优先"必须都是乘法

两个反直觉的坑，都是被真实结果逼出来的：

- **状态权重若是加数**，失效链接会被"百度优先 +0.3"和"有提取码 +0.1"抬到高分。
- **相关性与加成的关系**：如果相关性是乘法、加成是加法，一条相关性只有 0.53 的
  「地狱占星师 4K HDR」会拿到 0.83 分，**压过**相关性 1.0 的「沙丘2 4K HDR」（0.8 分）。
  实测「沙丘 4K HDR」的头几条全是这种不相关结果。

现在所有加成都是乘法因子，状态权重最后相乘。

### 5️⃣ 多词查询会召回塌陷

实测「沙丘 4K HDR」在聚合引擎只有 **3~25 条**，而「沙丘」有 **180+ 条**。
所以原始查询召回不足时会自动补搜主词，并对补搜结果降权 50%，
既拿到召回又不让噪声淹没主查询结果（摘要里会明确显示补搜了什么）。
...
│   ├── score.py              # 排序打分（相关性/状态/网盘优先，全部乘法）
│   ├── store.py              # SQLite 验活缓存 / 搜索日志
│   ├── pipeline.py           # 并发编排（单源失败隔离 + 多词查询自动补搜）
│   ├── cli.py                # CLI
│   ├── webapp.py             # Web API
...
| 限制 | 说明 / 下一步 |
|---|---|
| **公共 PanSou 实例不稳定** | 实测偶发 400 / timeout / **429 限流**（开发期间被限过一次）。已做多实例 + 重试降级。**下一步自建 Docker 版 PanSou**，这是提升覆盖最有效的一步 |
...
| **大结果集会慢** | 700+ 条链接全量验活约 1~2 分钟（已按网盘分别限速）。二次搜索走缓存，通常几秒 |
...
```bash
.venv/bin/python -m pytest -q        # 105 个测试，全离线，2 秒跑完
```
