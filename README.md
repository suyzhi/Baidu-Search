...
# 第一次用：把 TG 频道索引建起来（召回主力，越深越全）
.venv/bin/pansearch index crawl --pages 3            # 136 频道 × 3 页 ≈ 30 秒
.venv/bin/pansearch index crawl --pages 30 --deepen  # 继续向历史深挖
.venv/bin/pansearch index stats
```

> 首次搜索若索引为空会自动抓一轮最新消息。但**索引越深召回越高**：
> 只抓 3 页时「沙丘 4K HDR」命中 265 条，挖到 33 页后同一关键词能出 1805 条。
...
### 6️⃣ 召回瓶颈是"数据源深度"，不是关键词

网盘链接的主要产地是 **Telegram 频道**。聚合引擎本质也只是在抓它们，
但公共实例限流、超时、还随时可能 403，一次只给你它当次抓到的那点。
...
| 关键词 | 索引 6.5k 条时 | 索引 55k 条时 |
|---|---|---|
| 沙丘 4K HDR | 265 条 | **1805 条** |
...
| PanSou 聚合引擎 | 8~25s | 170~330 条 |
| Brave / 360 搜索引擎 | 12~22s | 3~6 条 |

按各源配置独立 `deadline`（PanSou 25s / Brave 12s），超时跳过、其余结果照常返回；
再配合"先排序、只验活最相关的前 300 条"的**验活预算**（700+ 条全量验活要 1~2 分钟，
而用户只看前几十条），实测 **47s → 17s**。
...
| 限制 | 说明 / 下一步 |
|---|---|
| **公共 PanSou 实例越来越不稳** | 实测偶发 400 / timeout / **429 / 403**（开发期间被限过多次）。已做多实例 + 重试 + 25s 超时降级；TG 索引已能独立支撑召回。**下一步自建 Docker 版 PanSou** |
| **Brave 会 429** | 已实现限速 + 本轮冷却 + 12s 超时。长期方案：自建 SearXNG 实例（支持 JSON API，可聚合多引擎） |
| **TG 索引需要时间养** | 只抓最新 3 页时召回有限，挖到 33 页才显著。建议定期跑 `index crawl --deepen`（可挂 cron）。索引在 `.cache/`，不进 git |
...
```bash
.venv/bin/python -m pytest -q        # 126 个测试，全离线，2 秒跑完
```
